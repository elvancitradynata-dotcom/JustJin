"""
╔══════════════════════════════════════════════════════╗
║       CRYPTO SCALPING SIGNAL BOT v2.1 Railway       ║
    ...
    try:
        async with session.get(BYBIT_BASE + TICKER_EP, params=params,
            ...
            if data.get("retCode") == 0:
                ...
                return {
                    "last": float(t["lastPrice"]), "mark": float(t["markPrice"]),
...
def score_signal(ind):
    ...
    # Volume
    if vs >= 2.0:   sl+=8; ss+=8; reasons.append(f"🔥 Volume spike {vs}x")
    elif vs >= 1.5: sl+=4; ss+=4; reasons.append(f"📢 Volume elevated {vs}x")
        ...
        for symbol in WATCHLIST:
            try:
                ...
                for tf in [5, 15]:
                    ...
                    if conf < 60 or direction == "NEUTRAL": continue
...
async def before_scan():
    ...
    print(f"⏳ Mulai scan pertama dalam {SCAN_INTERVAL}s...")
...
async def cmd_price(ctx, symbol: str = "BTCUSDT"):
    ...
    if not t:
        await ctx.send(f"❌ `{symbol}` tidak ditemukan."); return
    pct = t["pct24h"]
